...
3. Кожна ідея має бути КОНКРЕТНОЮ та ЗНІМАБЕЛЬНОЮ — не загальна тема, а чітка концепція
   відео з ясним сюжетом/конфліктом/інтригою. Глядач має одразу зрозуміти, ЩО він побачить.
   Уникай банальних і шаблонних ідей ("топ-5 порад", "як стати кращим"). Шукай несподівані
   кути, особисті виклики, експерименти, контрасти, провокаційні питання.
...
5. БУДЬ ЛАКОНІЧНИМ, але ЗМІСТОВНИМ: title_patterns — рівно 2 пункти, кожен до 8 слів.
   content_tone — одне речення до 10 слів. hook — одне яскраве речення-гачок до 15 слів.
   summary — 2-3 речення (до 50 слів) з КОНКРЕТНИМ описом: що відбувається у відео,
   який ключовий поворот/конфлікт, чим закінчується. НЕ використовуй вкладені масиви.
...
Формат відповіді:
{
  ...
  "video_ideas": [
    {
      "title": "конкретний заголовок під нішу користувача",
      "format": "короткий опис формату",
      "hook": "яскраве речення-гачок",
      "summary": "2-3 речення з конкретним сюжетом відео"
    }
  ]
}

Згенеруй рівно 3 СИЛЬНІ, конкретні ідеї у video_ideas.`;
...
app.post('/generate-script', async (req, res) => {
  try {
    ...
    if (targetDuration) {
      // Орієнтир: ~2.5 слова за секунду української начитки
      const targetWords = Math.round(targetDuration * 2.5);
      if (targetDuration <= 60) {
        durationInstruction = `\n\nКРИТИЧНО ВАЖЛИВО — ТРИВАЛІСТЬ: відео має тривати РІВНО ${targetDuration} секунд.
Весь текст озвучення (voiceover усіх сцен разом) має містити приблизно ${targetWords} слів — не більше.
Це КОРОТКЕ відео: ${targetDuration <= 20 ? '1 сцена' : targetDuration <= 40 ? '1-2 сцени' : '2-3 сцени'}, короткі динамічні репліки.
НЕ перевищуй ліміт слів. estimated_duration = "${targetDuration} сек".`;
      } else {
        const mins = Math.round(targetDuration / 60);
        durationInstruction = `\n\nТРИВАЛІСТЬ: приблизно ${mins} хвилин (${targetDuration} сек).
Весь текст озвучення має містити приблизно ${targetWords} слів. Розгорнутий сценарій з кількома сценами.
estimated_duration = "${mins} хв".`;
      }
    }
